...
import { catchError as catchError3, concat, concatMap, EMPTY, firstValueFrom, from as from2, of as of2, ReplaySubject as ReplaySubject2, scan as scan2 } from "rxjs";
    ...
    remoteAgentHandler: async ({ actionInputsWithoutAgents, threadId }) => {
      ...
      return agent.legacy_to_be_removed_runAgentBridged({
        ...
        if (event.type === RuntimeEventTypes.RunError) {
          const { message } = event;
          return throwError(() => new CopilotKitError5({
            message,
    ...
    return this.eventStream$.pipe(
      ...
      concatMap((eventWithState) => {
        if (eventWithState.event.type === "ActionExecutionEnd" && eventWithState.callActionServerSide) {
          ...
          return concat(of2(eventWithState.event), toolCallEventStream$).pipe(catchError3((error) => {
            const structuredError = ensureStructuredError3(error, convertStreamingErrorToStructured2);
            if (this.errorHandler && this.errorContext) {
          ...
          return of2(eventWithState.event);
        }
      })
    );
  }
